...
-  (void)drawDashLines {
    ...
    //底部实线
    UIView *bottomLine = [[UIView alloc]initWithFrame:CGRectMake(START_X_VALUE, vHeight-25, vWidth-2*START_X_VALUE, _bottomLineHeight)];
    ...
    if (XAxisIndexs.count >0 && self.showFiveDates.count > 0) {
        for (int i = 0; i<self.showFiveDates.count; i++) {
...
//            [self addSubview:xLineView];
            ...
            xAxisLabel.text = self.showFiveDates[i];
            ...
            xAxisLabel.center = CGPointMake(30+i*width, vHeight-xAxisLabel.bounds.size.height/2);
