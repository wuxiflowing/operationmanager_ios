//
//  JKControllerConfigurationfourCell.m
//  OperationsManager
//
...
@interface JKNewControllerConfigurationFourCell () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate>
...
@implementation JKNewControllerConfigurationFourCell
...
- (void)setDataSource:(NSMutableArray *)dataSource {
    ...
    self.oxygenUpD = [NSString stringWithFormat:@"%.1f",model.controlInfo4.oxyLimitUp];
    self.oxygenDownD =[NSString stringWithFormat:@"%.1f",model.controlInfo4.oxyLimitDown];
    self.electricCurrentUpD = [NSString stringWithFormat:@"%.1f",model.controlInfo4.electricityUp];
    self.electricCurrentDownD = [NSString stringWithFormat:@"%.1f",model.controlInfo4.electricityDown];
...
- (void)textFieldDidChangeValue:(NSNotification *)notification {
    if (_dataSource.count == 0) {
        ...
        if (textField.tag == 1) {
            self.oxygenUpD = textField.text;
        } else if (textField.tag == 2) {
            self.oxygenDownD = textField.text;
        } else if (textField.tag == 3) {
            self.electricCurrentUpD = textField.text;
        } else if (textField.tag == 4) {
            self.electricCurrentDownD = textField.text;
        }
    } else {
        ...
        if (textField.tag == 1) {
            self.oxygenUpD = textField.text;
            model.voltageUpB = self.oxygenUpD;
        } else if (textField.tag == 2) {
            self.oxygenDownD = textField.text;
            model.voltageDownB = self.oxygenDownD;
        } else if (textField.tag == 3) {
            self.electricCurrentUpD = textField.text;
            model.electricCurrentUpB = self.electricCurrentUpD;
        } else if (textField.tag == 4) {
            self.electricCurrentDownD = textField.text;
            model.electricCurrentDownB = self.electricCurrentDownD;
        }
    }
}
    ...
    if (indexPath.row == 0) {
        ...
        if (indexPath.row == 1 || indexPath.row == 2) {
            ...
            if (indexPath.row == 1) {
                ...
                if (self.oxygenUpD != nil) {
                    self.oxygenUpATF.text = self.oxygenUpD;
                }
            } else {
                [self.oxygenDownATF removeFromSuperview];
                self.oxygenDownATF = textField;
                if (self.oxygenDownD != nil) {
                    self.oxygenDownATF.text = self.oxygenDownD;
                }
            }
            ...
            if (indexPath.row == 3) {
                ...
                if (self.electricCurrentUpD != nil) {
                    self.electricCurrentUpATF.text = self.electricCurrentUpD;
                }
            } else {
                [self.electricCurrentDownATF removeFromSuperview];
                self.electricCurrentDownATF = textField;
                if (self.electricCurrentDownD != nil) {
                    self.electricCurrentDownATF.text = self.electricCurrentDownD;
                }
            }
        }
    }else{
